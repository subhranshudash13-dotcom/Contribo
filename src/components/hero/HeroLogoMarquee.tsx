...
function KubernetesIcon() {
  return (
    <svg viewBox="0 0 24 24" className="w-full h-full object-contain" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="w-full h-full object-contain" xmlns="http://www.w3.org/2000/svg" aria-hidden>
...
function GitHubIcon() {
  return (
    <svg viewBox="0 0 24 24" className="w-full h-full object-contain text-primary" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden>
...
type LogoItem = { name: string; logo: React.ReactNode };

function LogoRow({ items, keyPrefix }: { items: LogoItem[]; keyPrefix: string }) {
  return (
    <div className="flex animate-scroll-right-slow w-max">
      {[0, 1].map((dup) => (
        <div key={`${keyPrefix}-dup-${dup}`} className="flex gap-6 lg:gap-8 pr-6 lg:pr-8">
          {items.map((item, idx) => (
            <div
              key={`${keyPrefix}-${dup}-${idx}`}
              title={item.name}
              className="w-[56px] lg:w-[64px] aspect-square flex items-center justify-center transition-all duration-300 hover:scale-110 cursor-pointer group bg-transparent border-none shadow-none"
            >
...
/**
 * Decorative program/org logo marquee for the hero.
 * Split out so it can be code-split and deferred without blocking LCP text.
 */
export function HeroLogoMarquee() {
  const COL_1: LogoItem[] = [
    { name: 'GSoC', logo: <GSoCLogo color className="w-full h-full object-contain" /> },
    { name: 'Kubernetes', logo: <KubernetesIcon /> },
    { name: 'Outreachy', logo: <OutreachyLogo color className="w-full h-full object-contain" /> },
    { name: 'Python', logo: <PythonIcon /> },
    { name: 'LFX Mentorship', logo: <LFXLogo color className="w-full h-full object-contain" /> },
    { name: 'European Summer of Code', logo: <ESoCLogo color className="w-full h-full object-contain" /> },
    { name: 'GitHub', logo: <GitHubIcon /> },
  ];

  const COL_2: LogoItem[] = [
    { name: 'CNCF', logo: <CNCFIcon /> },
    { name: 'MLH Fellowship', logo: <MLHLogo color className="w-full h-full object-contain" /> },
    { name: 'Docker', logo: <DockerIcon /> },
    { name: 'Hacktoberfest', logo: <HacktoberfestLogo color className="w-full h-full object-contain" /> },
    { name: 'Google', logo: <GoogleIcon /> },
    { name: 'Summer of Bitcoin', logo: <SummerOfBitcoinLogo color className="w-full h-full object-contain" /> },
  ];

  const COL_3: LogoItem[] = [
    { name: 'TensorFlow', logo: <TensorFlowIcon /> },
    { name: 'Apache', logo: <ApacheIcon /> },
    { name: 'Linux Foundation', logo: <LinuxFoundationIcon /> },
    { name: 'GSoC', logo: <GSoCLogo color className="w-full h-full object-contain" /> },
    { name: 'Python', logo: <PythonIcon /> },
    { name: 'Kubernetes', logo: <KubernetesIcon /> },
  ];

  const COL_4: LogoItem[] = [
    { name: 'Linux Foundation', logo: <LinuxFoundationIcon /> },
    { name: 'Hacktoberfest', logo: <HacktoberfestLogo color className="w-full h-full object-contain" /> },
    { name: 'Google', logo: <GoogleIcon /> },
    { name: 'CNCF', logo: <CNCFIcon /> },
  ...
  return (
    <div
      className="lg:col-span-5 relative h-[402px] lg:h-[502px] w-full hidden lg:flex flex-col items-center justify-center overflow-hidden select-none mt-[57px] sm:mt-[89px] lg:-mr-12"
      aria-hidden
    >
      <div className="flex flex-col justify-center gap-6 lg:gap-8 w-[150%] h-full rotate-0">
        <LogoRow items={COL_1} keyPrefix="col1" />
        <LogoRow items={COL_2} keyPrefix="col2" />
        <LogoRow items={COL_3} keyPrefix="col3" />
        <LogoRow items={COL_4} keyPrefix="col4" />
      </div>
    </div>
  );
}
